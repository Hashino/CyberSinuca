...
const History: React.FC = () => {
  ...
  const { data: apiResponse, isLoading } = useQuery<ResponseProps>({
    ...
    queryFn: async () => {
      const response = await fetch(`http://localhost:8888/matches?_page=${page}&_per_page=10`);
  ...
  return (
    <BasicLayout>
      <div className="flex h-full w-full flex-col px-4">
        <div className="py-4">
          <h1 className="px-4 py-2 text-2xl font-bold text-gray-3 dark:text-gray-1">HISTÓRICO</h1>
          <div className="h-fit w-full rounded-md bg-gray-1 shadow-md dark:bg-dark-2">
            ...
            <div className="w-full">
              <HistoryTable matches={apiResponse?.data} />
            </div>
          </div>
        </div>
      </div>

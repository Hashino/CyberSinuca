...
const UserSelection: React.FC<ComboboxProps> = ({ options, selectedUser, setSelectedUser }) => {
  const [query, setQuery] = useState<string>('');
  const filteredOptions =
    query === ''
      ? options
      : options.filter((search) => {
          return search.name.toLocaleLowerCase().includes(query.toLocaleLowerCase());
        });

  return (
    <Combobox value={selectedUser} onChange={setSelectedUser} onClose={() => setQuery('')}>
      <div className="group relative text-dark-3 transition dark:text-light">
        <ComboboxInput
          aria-label="Assignee"
          displayValue={(search: UserType) => search?.name}
